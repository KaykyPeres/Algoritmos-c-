...
int main()
{
	int pConstelacao; // numero de pontos existentes em uma constelaçao
	ponto p[100];
	
	//entrando o numero de pontos
	cin >> pConstelacao;
	
	//entrando comas coordenadas
	for(int i=0; i<pConstelacao; i++)
	{
		cin >> p[i].x >> p[i].y;
	} 
	
	cout<< Graham_scan(p,pConstelacao).size() << " Lados" << endl;
